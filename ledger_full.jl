...
const COST_ACCOUNT_INFO = Dict(
    ...
    :pharmacy_supply_costs => "Cost of sales: Purchases-COS"
)
...
# builds the folder names from whatever was typed in
ledger_dir  = "$(y)_ledgers"
daily_dir   = "$(y)_daily_records"

# creates folder if it doesn't exist; does nothing (no error) if it already does,
mkpath(ledger_dir)
mkpath(daily_dir)

df_ledger = write_ledger_csv(monthly_costs, monthly_total, joinpath(ledger_dir, "ledger_$(m)_$(y).csv"))
df_daily  = write_daily_records_csv(daily_values, date, joinpath(daily_dir, "daily_records_$(m)_$(y).csv"))
log_entry()  # records who ran this script and when, appended to audit_log.txt in pwd()

println("Done. Files written to: $(pwd())")using DataFrames, CSV, Dates
