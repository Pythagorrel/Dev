...
function staged_records()
    df = load_staged()
    # CHANGED IN v4.0: JOURNAL_KEYS rather than KEY_ORDER, so the balances and
    # the variance columns survive a round trip through the notepad, and the
    # status and reason are carried too. Without this a day staged as "closed",
    # or a difference the operator had already explained, would silently revert
    # to a blank trading day on commit.
