...
"A fully-zeroed amounts dict covering every configured category."
blank_amounts() = Dict{Symbol,Float64}(k => 0.0 for k in KEY_ORDER)
...
function read_day_csv(path::AbstractString)
    ...
    # --- amounts ------------------------------------------------------------
    amounts = blank_amounts()          # start fully zeroed...
    for k in KEY_ORDER                 # ...then overwrite only what was supplied
        col = colname_of(k)
        if hasproperty(row, Symbol(col))
            amounts[k] = _as_float(getproperty(row, Symbol(col)))
        end
    end

    # Unknown columns are reported rather than ignored: a typo'd header would
    # otherwise be silently dropped and read as a zero.
    known = Set(vcat(["year", "month", "day", "Date"], [colname_of(k) for k in KEY_ORDER]))
    for c in names(df)
